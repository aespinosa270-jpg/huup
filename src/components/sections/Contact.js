...
// Eliminamos la importación inestable de useActionState y useFormStatus
// Importamos solo las dependencias estables:
import { useState, useEffect } from "react"; 
import { sendContactEmail } from "@/app/actions"; // Función de envío REAL (Server Action)
...
// Componente de Ícono WhatsApp (se mantiene)
...
// Componente SubmitButton adaptado para recibir el estado `pending` como prop
function SubmitButton({ isPending }) {
    return (
        ...
        >
            ...
            {!isPending && <div className="absolute inset-0 bg-white/20 -translate-x-full group-hover:translate-x-0 transition-transform duration-300 skew-x-12 origin-left" />}
        </button>
    );
}

// ContactDataLink (se mantiene)...
...
export default function Contact() {
    // Reemplazamos useActionState por estados locales estables
    const [state, setState] = useState(initialState); 
    const [latency, setLatency] = useState(null);
    const [isPending, setIsPending] = useState(false); // Nuevo estado para el botón

    // Lógica de Latencia (se mantiene)
    useEffect(() => {
    ...
    // NUEVO HANDLER DE ENVÍO para reemplazar formAction
    const handleSubmit = async (event) => {
        event.preventDefault(); // Detiene el envío nativo (crucial)

        setIsPending(true); // Activa el spinner del botón
        setState(initialState); // Limpia mensajes anteriores
        ...
        setState(result);
        setIsPending(false); // Desactiva el spinner

        // Si la misión fue exitosa, resetear el formulario (buena UX)
        if (result.success) {
            event.target.reset();
        }
    };
    ...
    return (
        <section id="contacto" className="py-32 bg-brand-dark relative overflow-hidden">
            ...
            {/* Fondo Grid Continuo... (se mantiene) */}
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                    ...
                    {/* === COLUMNA IZQUIERDA (Se mantiene intacta) === */}
                    <div>
                        {/* ... (Markup de la columna izquierda se mantiene) ... */}
                        <motion.div 
                    ...
                    {/* === COLUMNA DERECHA: FORMULARIO AMIGABLE (CORREGIDO) === */}
                    <motion.div
                    ...
                    >
                        ...
                        <div className="bg-black/80 backdrop-blur-xl p-8 md:p-10 rounded-3xl border border-white/10 relative shadow-[0_0_50px_rgba(0,0,0,0.5)] min-h-[500px] flex flex-col">
                            ...
                            {/* CABECERA CON PING REAL (Se mantiene) */}
                            ...
                            <AnimatePresence mode="wait">
                                {state.success ? (
                                    /* === ESTADO DE ÉXITO: TERMINAL STYLE (Se mantiene) === */
                                    <motion.div 
                                        ...
                                        <div className="bg-black border border-green-500/30 p-6 rounded font-mono text-sm shadow-[0_0_30px_rgba(0,255,0,0.1)]">
                                            ...
                                            <div className="space-y-2 text-green-400">
                                                ...
                                                <p className="text-gray-400 text-xs">
                                                    // NOTA_SISTEMA: El equipo de ingeniería revisará tu solicitud.
                                    ...
                                    /* === FORMULARIO STANDARD (CORREGIDO) === */
                                    <motion.form 
                                        key="contact-form"
                                        onSubmit={handleSubmit} // USAMOS EL HANDLER STABLE DE ONSUBMIT
                                    ...
                                    >
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <input 
                                                type="text" id="company" name="company" placeholder="Ej. Pizzería La Napolitana S.A." 
                                                className="w-full bg-black border border-white/20 rounded-none px-4 py-4 text-white placeholder:text-gray-700 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-all font-mono text-sm" 
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <textarea 
                                            id="message" name="message" required rows="5" placeholder="Necesito un sistema de pedidos en línea, un catálogo de productos y que sea muy rápido en el celular." 
